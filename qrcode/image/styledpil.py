...
class StyledPilImage(qrcode.image.base.BaseImageWithDrawer):
    ...
    def new_image(self, **kwargs):
        ...
        return Image.new(mode, (self.pixel_width, self.pixel_height), back_color)
    ...
    def draw_embedded_image(self):
        if not self.embedded_image:
            return
        # QR code is centered on the fixed canvas
        # Use pixel_size (square QR dimension) for calculations
        qr_size = self.pixel_size
        logo_width_ish = int(qr_size * self.embedded_image_ratio)
        # Calculate logo offset relative to QR code position
        logo_offset_in_qr = (
            int((int(qr_size / 2) - int(logo_width_ish / 2)) / self.box_size)
            * self.box_size
        )  # round the offset to the nearest module
        # Absolute position on canvas
        logo_offset_x = self.qr_offset_x + logo_offset_in_qr
        logo_offset_y = self.qr_offset_y + logo_offset_in_qr
        logo_position = (logo_offset_x, logo_offset_y)
        logo_width = qr_size - logo_offset_in_qr * 2
