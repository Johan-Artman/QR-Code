...
class SvgFragmentImage(qrcode.image.base.BaseImageWithDrawer):
    ...
    def _svg(self, tag=None, version="1.1", **kwargs):
        ...
        width_dimension = self.units(self.pixel_width)
        height_dimension = self.units(self.pixel_height)
        return ET.Element(
            tag,
...
class SvgPathImage(SvgImage):
    ...
    def _svg(self, viewBox=None, **kwargs):
        if viewBox is None:
            width_dimension = self.units(self.pixel_width, text=False)
            height_dimension = self.units(self.pixel_height, text=False)
            viewBox = f"0 0 {width_dimension} {height_dimension}"
